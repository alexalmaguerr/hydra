...
**2026-04-20 (precarga paso Configuración):** al reanudar con `procesoId`, si existe solicitud vinculada al contrato del proceso, el paso **Configuración** (`PasoConfigContrato`) precarga **administración**, **tipo de contratación**, **actividad** y **referencia contrato padre** desde `formData` de la solicitud (con `GET /tipos-contratacion/:id` para bandera `esIndividualizacion` y descripción). **Distrito** solo se muestra y exige selección cuando el tipo es individualización; en otros casos queda oculto y no se envía `distritoId` en `variablesCapturadas` salvo que aplique.
