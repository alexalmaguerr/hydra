...
### MOD-T03-1: Trigger automático post-instalación de toma (Req 5)

**Estado anterior:** `Orden` se crea manualmente.  
**Cambio:** Event listener que al cambiar `Orden` de tipo `InstalacionToma` a estado `Ejecutada`, genera automáticamente una `Orden` de tipo `InstalacionMedidor` para el mismo contrato.
...
### MOD-T03-2: Tipología de cortes ampliada (Req 11)

**Estado anterior:** `Orden.tipo` incluye `Corte` sin subtipología.  
**Cambio:** Campo `subtipoCorteId` en `Orden` vinculado a `CatalogoTipoCorte` (definido en T11).
...
- [ ] Frontend: selector de subtipo visible cuando tipo === 'Corte'

### MOD-T03-3: Atributos de corte — cortable/no cortable (Req 12)

**Estado anterior:** `Orden.datosCampo` es JSON genérico.  
**Cambio:** Campos explícitos para condiciones de corte.
...
- [ ] Frontend: campo ubicación de corte con opciones predefinidas

### Nuevas dependencias

| Dependencia | Modelo | Descripción |
|---|---|---|
| T11 | `CatalogoTipoCorte` | Catálogo de subtipos de corte |
| T10 | `ProcesoContratacion` | Coordinar con flujo E2E para auto-generar orden medidor |
