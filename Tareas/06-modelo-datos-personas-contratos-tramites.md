...
### MOD-T06-1: Ampliar modelo Persona (Req 20)

**Estado anterior:** Persona tiene: nombre, rfc, tipo, email, telefono.  
**Cambio:** Campos adicionales para cumplir con el PRD.
...
- [ ] Actualizar DTOs de PersonasService para incluir campos nuevos
- [ ] Actualizar frontend: formularios de persona con apellidos, CURP, régimen fiscal
- [ ] Validar formato CURP (18 caracteres) y RFC (12–13 caracteres)
- [ ] Agregar índice por CURP (ya aplicado en schema)

### MOD-T06-2: Agregar rol "Contacto" (Req 2)

**Estado anterior:** Roles: Propietario, Cliente, PersonaFiscal, Arrendatario.  
**Cambio:** Agregar `Contacto` como rol válido (persona designada para comunicaciones operativas).

- [ ] Agregar `Contacto` a la validación de roles en `asignarRol()`
- [ ] Frontend: opción Contacto en selector de rol al vincular persona

### MOD-T06-3: Vista E2E del flujo contractual (Reqs 1, 3, 4)

**Estado anterior:** `HistoricoContrato` registra cambios campo por campo.  
**Cambio:** Vista agregada de secuencia operativa.

- [ ] Agregar endpoint `GET /contratos/:id/flujo-completo` que agregue:
  - Historial del contrato
  - Órdenes asociadas
  - Trámites vinculados
  - Hitos de contratación (T10)
  - Personas con roles activos
- [ ] Frontend: componente `FlujoProcesual.tsx` con timeline visual

### MOD-T06-4: Vincular Contrato con Domicilio (Req 21 parcial)

**Estado anterior:** `Contrato.direccion` es string plano.  
**Cambio:** Relación con modelo Domicilio (depende de T12).
...
- [ ] Script de migración: parsear `Contrato.direccion` → crear `Domicilio` y vincular
- [ ] Mantener `direccion` como campo legacy durante transición
- [ ] Frontend: mostrar domicilio estructurado cuando disponible, fallback a string

### MOD-T06-5: Vincular Contrato con PuntoServicio (Req 9 parcial)

**Estado anterior:** `Contrato.tomaId` apunta a una toma.  
**Cambio:** Nuevo campo `puntoServicioId` (depende de T11).
...
- [ ] Script de migración: `Toma` → `PuntoServicio` y vincular contratos
- [ ] Mantener `tomaId` como campo legacy durante transición

### MOD-T06-6: Vincular Contrato con TipoContratacion (Req 22 parcial)

**Estado anterior:** `Contrato.tipoContrato` es string plano.  
**Cambio:** Relación con modelo `TipoContratacion` (depende de T13).
...
- [ ] Script de migración: mapear `tipoContrato` string → `TipoContratacion` ID
- [ ] Mantener `tipoContrato` como campo legacy durante transición

### MOD-T06-7: Relación Contrato → ProcesoContratacion (Req 1)
...
- [ ] Exponer en endpoint de detalle de contrato

### Nuevas dependencias

| Dependencia | Modelo | Descripción |
|---|---|---|
| T10 | `ProcesoContratacion` | Flujo E2E de contratación |
| T11 | `PuntoServicio` | Entidad separada de toma |
| T12 | `Domicilio`, `DomicilioPersona` | Domicilios homologados INEGI |
| T13 | `TipoContratacion` | Tipos parametrizados |
